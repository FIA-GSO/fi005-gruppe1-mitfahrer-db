  ...
  >
    ...
    >
      ...
      <a
        href="#"
        v-if="userStore.user"
        @click="$refs.userPopup.showPopup()"
        class="p-2 text-2xl hover:bg-white hover:text-gso-blue rounded-full w-12 h-12"
      >
        <div>
          <img
      ...
      <Popup ref="userPopup">
        <div
          class="flex flex-col bg-white text-black items-end sm:items-center w-screen sm:w-64 sm:rounded-bl-lg"
        >
          <div class="flex items-center w-full h-16 mb-4">
            ...
            <a
              href="http://127.0.0.1:5000/help.pdf"
              @click="$refs.userPopup.hidePopup()"
              class="p-2 text-2xl rounded-full w-12 h-12"
            >
              <font-awesome-icon
                v-if="userStore.user"
          ...
          <RouterLink
            :to="{ name: 'userDetails' }"
            @click="$refs.userPopup.hidePopup()"
            class="text-sm mb-4 pr-4 sm:pr-0"
            >Personendaten bearbeiten</RouterLink
          >
          <a
            href="#"
            @click="logout"
            class="text-sm mb-4 pr-4 sm:pr-0 text-red-500"
            >Ausloggen</a
          >
        </div>
      </Popup>
